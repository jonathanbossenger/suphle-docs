...
When the application is published [using the RoadRunner binary](/docs/v2/application-server), the same modules [bundled for routing](/docs/v2/modules#Connecting-standalone-modules) will be passed to passed to each dedicated task worker. The number of workers is controlled from the RoadRunner congig `*-rr.yaml` file.
